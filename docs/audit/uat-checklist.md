...
- Worktree backend: `.claude/worktrees/audit-nonprod` (đã checkout `merge/main-into-v3` @ commit cuối trong report — 2026-09-19 (SHORT JOB): `60982f9`; nếu `git -C .claude/worktrees/audit-nonprod log -1` khác thì `git checkout --detach merge/main-into-v3` ở đó rồi khởi động lại server + `curl -X POST localhost:3101/api/chat -d '{}'` phải trả 400).
...
- APK debug build 2026-09-19 17:06 từ `43d37d2` (source Android không đổi từ `766d31a` đến `60982f9`), trỏ về `http://10.0.2.2:3101/` + Supabase audit, ĐÃ cài và mở được màn đăng nhập (guest identity đã `pm clear`): `android/app/build/outputs/apk/debug/app-debug.apk` (cài lại: `adb -s emulator-5554 install -r <apk>`).
...
| 25 | **Nhãn in đậm — luật DƯƠNG (A.1)** | Bất kỳ bold nào không phải TÊN RIÊNG (`**Lưu ý:**`, `**Gợi ý?**`, `**Bữa trưa:**`, `**Tổng kết**`, `**Bạn muốn ăn gì?**`, `**Ưu tiên chính của bạn là gì?**`) đều KHÔNG bị cắt — thay dòng 20. Tên quán thật in đậm (kể cả viết hoa/không dấu/khoảng trắng khác hàng dữ liệu) vẫn được nhận; quán BỊA vẫn bị cắt. Log `tappyai_audit_grounding` chỉ được có tên quán, không có nhãn. |
| 26 | **Mở khuya = giờ mở cửa của hàng (A.2)** | `Spa nào mở khuya sau 22h ở Quận 3` / `quán ăn nào gần Quận 1 còn mở giờ này không` ⇒ quán được chọn PHẢI có `opening_hours` đóng ≥ 23:00 / qua đêm / cả ngày (card hiện giờ); quán đóng 22:00 chỉ được nêu là "đóng sớm hơn / nếu muốn đi sớm". Không hàng nào có giờ ⇒ câu "Mình chưa thấy bằng chứng về giờ mở khuya…". Log `step: attributes` có `hard_row_backed_by.late_open` = danh sách quán vouch. |
| 27 | **Khách sạn cũng qua cổng ràng buộc (A.3)** | `khách sạn Đà Nẵng có view biển, sang chút` / `resort Phú Quốc có chỗ đậu xe cho gia đình 4 người, cần phòng riêng` ⇒ có câu heads-up "chưa thấy bằng chứng về view / chỗ đậu xe, phòng riêng" (trước đây hotel không có). Log `step: attributes` với `tool: get_hotel_prices`; tool không áp dụng (vé máy bay, mua sắm…) khi có ràng buộc ⇒ log `hard_not_applicable`. |
