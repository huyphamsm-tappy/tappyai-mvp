...
//
//   · Đã thích  — `review_likes` rows of THIS user, then the reviews. Since
//                 20260915b_review_likes_private.sql the table is SELECT-own by
//                 RLS (like `review_saves`): another user's query returns nothing,
//                 by policy. This module is only ever called with the signed-in
//                 owner's id, so the policy and the caller agree.
