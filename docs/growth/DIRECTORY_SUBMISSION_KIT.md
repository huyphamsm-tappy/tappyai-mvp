...
| **Cốc Cốc Search — URL submission** (coccoc.com/search/console) | Vietnam's #2 search engine ("574+ million queries per month") | yes (no fee stated) | yes (Vietnamese searchers) | TRUE search discovery once indexed | no (captcha form) | 10 min: submit `/`, `/about`, `/scam-shield`, `/scam-shield/kich-ban`, the five hubs, `/extension` | **yes — first VN-specific action**. Sitemap is already discoverable via the `Sitemap:` line in robots.txt. |
| Vietnam startup / tech directories (Techfest–NATEC listings, Vietnam startup catalogs, local AI communities) | local ecosystem | free tiers exist on several — **verify each at submission** | some | entity + local backlink | no | 30 min each | pick 2–3 after release; never mass-submit |
