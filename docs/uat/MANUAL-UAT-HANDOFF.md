...
- **Session C residuals (2026-09-22, `uat/phase7-regressions`)** — F-043 (a numeric threshold may still appear in a second-hand advice answer; ~1 run in 4 the answer regresses to cosmetic-first — prompt-only rule), F-045 (one card set per turn on a trip: hotel cards absent when food/attraction cards won). CLOSED in the follow-up: F-044 (v2 attribution default ON), F-046 (db suites run on Windows again, ports unique, Music parity row retired — full suite 0 failed), F-047 (scam-checker wording), F-048 (type-based venue exclusion + unpriced rows). A plan reply may still end with a soft "Bạn muốn điều chỉnh gì không?" — allowed, not a re-ask.
