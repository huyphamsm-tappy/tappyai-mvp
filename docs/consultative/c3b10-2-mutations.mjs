...
const M = [
  ...
  { f: MG, n: 'M08 redact SUPPORTED prices too',
    from: "  const bad = claims.filter(c => c.verdict !== 'VERIFIED')",
    to: '  const bad = claims.slice()' },
  ...
  { f: MG, n: 'M14 break the "+" variant boundary again (S24+ passes as S24)',
    from: '|\\+/g', to: '|\\b\\+\\b/g' },
  // ── C3-B.10.2: the three fixed defects ──────────────────────────────────
  { f: MG, n: 'M17 F1: drop the prefix-currency pass entirely',
    from: '  for (const m of masked.matchAll(PREFIX_RE)) {', to: '  for (const m of [] as RegExpExecArray[]) {' },
  { f: MG, n: 'M18 F1: drop ₫ from the prefix units (Shopee\'s own format)',
    from: "const PREFIX_UNIT = '(₫|VNĐ|VND|US\\\\$|\\\\$)'", to: "const PREFIX_UNIT = '(VNĐ|VND|US\\\\$|\\\\$)'" },
  { f: MG, n: 'M19 F2: remove the never-empty fallback (sentence removal always wins)',
    from: '  if (HAS_LETTER.test(sentenceLevel)) return sentenceLevel', to: '  return sentenceLevel' },
  { f: MG, n: 'M20 F3: skip sentence-level removal, always cut only the amount',
    from: '  const spans = sentenceSpans(text)', to: '  const spans: Array<[number, number]> = []' },
  { f: MG, n: 'M21 F3: split sentences on every "." again (cuts numbers in half)',
    from: '    if (next !== undefined && !/\\s/.test(next)) continue', to: '' },
