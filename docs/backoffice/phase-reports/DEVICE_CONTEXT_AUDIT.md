...
- **Added fields map cleanly to native:** `manufacturer`→`Build.MANUFACTURER`/`'Apple'`; `pixel_ratio`→`DisplayMetrics.density`/`UIScreen.scale`; `color_scheme`→system dark-mode (`UiModeManager`/`UITraitCollection`); `network_type`→`ConnectivityManager`/`NWPathMonitor` (same `wifi`/`cellular`/`ethernet`/`none` vocabulary); `is_pwa`→`false` on native (not an installed PWA); `build_number`→`versionCode`/`CFBundleVersion`; `sdk_version`→native SDK version. No web-only assumption blocks native population. (Full table in the Architecture Decision.)

## 5. Unnecessary payload growth — **ACCEPTED, QUANTIFIED**

- Per event, `device_context` carries the 19-field contract (jsonb). Of these, 6 (`platform`, `os_name`, `os_version`, `device_type`, `app_version`, `build_number`) are **also** in flat columns — a value-level denormalization (~70–110 bytes/row), not duplicated logic. `build_number` is now **single-sourced**: the flat envelope field is projected from `device_context.build_number` (one env read, was two). No field outside the required 19-field contract is stored; the four latest fields (`build_number`/`sdk_version`/`network_type`/`is_pwa`) cost **zero** new columns/migration (jsonb).
...
- **P-1 (LOW):** the *combination* of `screen_width/height` + `pixel_ratio` + `color_scheme` + `timezone` + `browser_version` + `os_version` (+ the new `network_type`) is a mild device-fingerprint surface — standard for any analytics SDK, but worth naming. The four newest fields are **low-entropy**: `network_type`/`is_pwa`/`color_scheme` have a handful of values each, and `build_number`/`sdk_version` are app-wide constants (not device-distinguishing). None is PII (no email/phone/name; `country` stays server-derived and out of the contract). Mitigations (NOT implemented, noted for a future privacy pass): coarsen screen dims/pixel_ratio to buckets, drop `timezone`, truncate `browser_version` to major. Current posture is low-risk: server-side only (never exposed to other users), write-only via the service-role path, read only through RBAC-gated (`analyst`+) admin dashboards — and nothing reads `device_context` yet at all.
