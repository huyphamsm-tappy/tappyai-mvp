...
**What this table cannot claim.** Rows 4–11 are enforced by PostgreSQL and are verified here only as **structural assertions against the migration text**: they prove the SQL *says* the right thing, not that the database *does* it. Runtime proof, as shipped, is a read-only production check (`pg_proc.proacl`, `information_schema`) taken **after** the Owner applies the migration by hand.

> **Correction, 2026-08-13 — the reason given for structural-only evidence was wrong.** It was recorded as an impossibility: no local PostgreSQL, so production is the only reachable database. That is false. `supabase/tests/` boots a **real PostgreSQL 17.5** through the `embedded-postgres` devDependency — no Docker, no `psql`, no `DATABASE_URL` — and eight suites already use it, including `platform_owner_revoke` and `platform_hardening_phase0`, which prove **grants** at runtime. C8's SQL could be proven the same way. It is not, and that is now a **known gap with a known remedy**, not a limit of the environment. Recorded here because the Owner ratified structural-only evidence on a premise that has since been shown to be untrue.
