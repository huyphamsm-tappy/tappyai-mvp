...
| V2.2-2 | **Marketing Phase 2 — CRUD** | 🔴 **IMPLEMENTATION NOT STARTED** · 📄 contract **MERGED** — [`V2.2_MARKETING_PHASE2_CONTRACT.md`](V2.2_MARKETING_PHASE2_CONTRACT.md), [#230](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/230), deployed `8ca080e` | The contract-first gate is **met**: the contract landed before any code, and the five modules under `/admin/marketing/*` are still **real guards over a placeholder** — `requirePagePermission(...)` then `<ModuleComingSoon />`, 16 lines each, **0 API routes, 0 tables**. 🚨 Two gates remain and **neither is met**: **Q6 is OPEN** — does consent export belong to Marketing Phase 2 or to the existing export workstream, and who owns delivery — and **M-30 is UNSATISFIED / activation-blocking**, because **no export mechanism is implemented** — neither the bulk/admin/reporting export (`05` §14 · `23` Phase 5 · `03` Module 19) nor the per-user portability export that `33` §6 routes to `10` §5. Which of the two answers M-30, and whether `10` §5's *preferences* / *subscriptions* already cover consent, are both **[OWNER DECISION REQUIRED]** — see contract §2.1. Governance — consent, rolling 24h/7d caps, quiet hours, unsubscribe — is built **before** CRUD, per the contract's own safety rule |
