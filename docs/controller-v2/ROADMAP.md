...
|---|---|---|---|
| V2.2-1 | **Phase C — broadcast** | 🟡 **BACKEND LANDED, SENDING OFF** — contract [`V2.2_PHASE_C_BROADCAST_CONTRACT.md`](V2.2_PHASE_C_BROADCAST_CONTRACT.md) approved; [#218](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/218) merged and deployed 2026-08-31. **NOT complete**: `CONTROLLER_BROADCAST_ENABLED` unset so sending is refused · production dry run **NOT RUN** · first real broadcast **NOT AUTHORISED** · C-14 confirmation UI **NOT BUILT** · anonymous-eligibility gap **OPEN** · legacy `CRON_SECRET` route **NOT RETIRED** (O-4 = C, deliberately after a verified real send) | The gate is met: the contract existed and was approved before code. The remaining gates are Owner decisions, one per step — dry run, then first real send, then §14.2's retirement sequence. A broadcast is **irreversible**, so none of them is implied by the one before it |
