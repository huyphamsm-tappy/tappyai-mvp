...
|---|---|---|---|
| V2.2-1 | **Phase C — broadcast** | 🟢 **V2.2-1 CLOSED · SENDING OFF** — contract [`V2.2_PHASE_C_BROADCAST_CONTRACT.md`](V2.2_PHASE_C_BROADCAST_CONTRACT.md) approved; [#218](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/218) merged and deployed 2026-08-31. Dry runs and the **first real broadcast** completed the same day under separate Owner approvals — campaign `4454c6b4…`, audience 1, `accepted: 1`, ledger stamp verified on the delivered row, and the Owner confirmed the notification arrived on the device. `CONTROLLER_BROADCAST_ENABLED` was armed for that one send and **disarmed again**, proven by a subsequent real-send request returning 403. ✅ **V2.2-1 CLOSED**: C-14 confirmation UI **BUILT** ([#223](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/223)) · anonymous-eligibility gap **CLOSED — MEASURED ZERO** by a direct read of `auth.users.is_anonymous` on production 2026-09-01, with [#225](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/225) blocking new anonymous subscriptions · legacy `CRON_SECRET` route **RETIRED AND DELETED** — 410 on 2026-08-31 (#220), recorder fixed (#221), one-hour observation window with **0 unexpected hits**, deleted 2026-09-01 under separate Owner approval. `CRON_SECRET` itself is unchanged and still load-bearing for nine cron routes | The gate is met: the contract existed and was approved before code. The remaining gates are Owner decisions, one per step — dry run, then first real send, then §14.2's retirement sequence. A broadcast is **irreversible**, so none of them is implied by the one before it |
| V2.2-2 | **Marketing Phase 2 — CRUD** | 🟡 **BUILT, CANNOT SEND — NOT COMPLETE** · contract [#230](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/230) · code [#237](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/237)–[#243](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/243) | The contract-first gate was met and the build followed contract order — **governance before CRUD**: consent (absence = opted out, no backfill), global unsubscribe, rolling 24h/7d caps, quiet hours 22:00–07:00 VN, the 10-user privacy floor, Marketing's own audience builder, the delivery ledger, campaign CRUD and lifecycle, the dry-run + typed `BROADCAST` path, and retention pruning. 🚨 **Two gates remain and neither is met.** **Q6 is OPEN** — does consent export belong to Marketing Phase 2 or to the existing export workstream, and who owns delivery. **M-30 is UNSATISFIED / activation-blocking**, because **no export mechanism is implemented** — neither the bulk/admin/reporting export (`05` §14 · `23` Phase 5 · `03` Module 19) nor the per-user portability export that `33` §6 routes to `10` §5; which of the two answers M-30, and whether `10` §5's *preferences* / *subscriptions* already cover consent, are both **[OWNER DECISION REQUIRED]** (contract §2.1). Sending is refused by a **source constant**, `CONSENT_EXPORT_SATISFIED = false` — deliberately not an env flag, so opening it costs a commit and a review rather than a dashboard click. **DoD 8 unmet ⇒ V2.2-2 is not complete.** ⚠️ The migration is **not applied to production**; the three tables do not exist there yet |
