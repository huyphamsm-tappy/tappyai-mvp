...
| **Marketing Phase 2 — CRUD** | 🔴 **IMPLEMENTATION NOT STARTED** · 📄 contract **MERGED** | The contract now exists — [`V2.2_MARKETING_PHASE2_CONTRACT.md`](V2.2_MARKETING_PHASE2_CONTRACT.md), merged in [#230](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/230) and deployed at `8ca080e`. **The code is unchanged**: the five modules under `/admin/marketing/*` are still **real guards over a placeholder** — `requirePagePermission(...)` then `<ModuleComingSoon />`, 16 lines each, **0 API routes, 0 tables**. 🚨 A merged contract is **not** permission to build. Activation is blocked twice over: **Q6 is OPEN** (who owns delivering consent export) and **M-30 is UNSATISFIED / activation-blocking**. Implementation order is governance first — consent, rolling caps, quiet hours, unsubscribe — then CRUD |
