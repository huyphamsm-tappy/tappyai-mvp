...
| **V2.2 Phase C — dry runs + FIRST REAL BROADCAST** | — (no code; production operation) | ✅ **VERIFIED 2026-08-31, sending re-disabled** | Six dry runs, all `sent: false`. Two resolved audience **0** with fingerprint `e3b0c44298fc1c14` = `sha256("")` — verified independently, so an empty audience rather than a swallowed failure. After a device re-subscribed, campaigns `8bb38ee0…` and `cf4989cf…` both returned audience **1** and the **identical** fingerprint `25239948da52a311` with **different** campaign ids — C-24/C-25 determinism proved against the production database. **First real broadcast:** campaign **`4454c6b4-71a1-4778-bdb4-40f759189e9d`**, governed route only, `dryRun:false`, HTTP 200, **`accepted: 1`**, `status: completed`, sent **once, no replay**. Inbox **6 → 7** (delta +1); the new row is `type: broadcast` and its `data` carries **exactly one key**, `broadcast_campaign_id` — the C-35 ledger working in production, confirmed by inspecting the row rather than replaying. Response contains `accepted` and **not** "delivered". 🔑 **The Owner confirmed the notification appeared on the device.** Armed via `vercel redeploy` (never `vercel --prod`; served source stayed `85ca5ab`), **armed window ≈ 5 minutes**, then disarmed — proven by a later real-send request returning **403 `Broadcast sending is disabled`**, inbox 7 → 7 |

> ✅ **PHASE C (V2.2-1) IS NOW CLOSED ON EVERY COUNT.** The send path is proven end to end on one recipient; the **C-14 confirmation UI is BUILT** ([#223](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/223)) — compose → dry run → review the resolved audience → type `BROADCAST` → send, with the send control absent until a dry run returns; the **anonymous-eligibility gap is CLOSED** — **MEASURED ZERO** by a direct read of `auth.users.is_anonymous` on production, 2026-09-01 ([contract §15.3](V2.2_PHASE_C_BROADCAST_CONTRACT.md)), with [#225](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/225) blocking creation of new anonymous subscriptions; and the legacy `CRON_SECRET` route is **RETIRED AND DELETED** — 410 on 2026-08-31 ([#220](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/220)), the recorder’s cold-start drop fixed ([#221](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/221)), a one-hour observation window returning **0 unexpected hits**, then deleted 2026-09-01 ([#222](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/222)). `CRON_SECRET` is unchanged and still load-bearing for the cron routes.
>
> ⚠️ **THREE THINGS THIS DOES NOT SAY.** **Sending is still DISABLED** — `CONTROLLER_BROADCAST_ENABLED` is unset and a real send is refused 403. **V2.2 as a whole is NOT complete** — V2.2-2 (Marketing Phase 2) has its **contract merged and deployed** ([#230](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/230), `8ca080e`) and its **implementation NOT STARTED**: five modules × 16 lines, `requirePagePermission(...)` → `<ModuleComingSoon />`, **0 API routes, 0 tables**. Activation is blocked on two open items — **Q6 is OPEN** (who owns delivering consent export) and **M-30 is UNSATISFIED / activation-blocking**. A merged contract is a design gate met, **not permission to build**. And **“measured zero” is point-in-time and narrow**: the control query found **324 anonymous users, one holding a legacy profile**; zero satisfy all three conditions today, and #225 is what keeps it that way.
>
> Sending is **DISABLED** (`CONTROLLER_BROADCAST_ENABLED` unset — measured, 0 matches across 43 production variables). It was armed once, for one send, and turned off again.
>
> Each remaining step is its own Owner decision and **none is implied by the one before it**. A broadcast cannot be recalled, which is why merging did not authorise deploying, deploying did not authorise enabling, enabling did not authorise a second send — and one verified send does not authorise the next.
...
| **Marketing Phase 2 — CRUD** | 🟡 **BUILT, CANNOT SEND** · contract [#230](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/230) · code [#237](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/237) [#238](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/238) [#239](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/239) [#241](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/241) [#242](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/242) [#243](https://github.com/huyphamsm-tappy/tappyai-mvp/pull/243) | Built in contract order, governance first: three tables with rollback and grants · the `'marketing'` notification category · a pure governance engine (consent absence = opted out, global unsubscribe, rolling 24h/7d caps, quiet hours 22:00–07:00 VN, the 10-user privacy floor) · the consumer consent control in EN/VI · Marketing's own audience builder (`broadcastAudience.ts` untouched, C-21) · the delivery ledger that IS both the cap's history and the idempotency ledger · campaign CRUD with a lifecycle whose `completed` has no outgoing edge · the dry-run + typed `BROADCAST` activation path · retention pruning. 🚨 **IT CANNOT SEND, BY DESIGN.** `CONSENT_EXPORT_SATISFIED` is a **source constant set to `false`** — not an env flag, because M-30 is an unmet legal precondition rather than an operational setting — so a real send is refused in the route **and again inside the runner**. **Q6 OPEN · M-30 UNSATISFIED · DoD 8 unmet ⇒ V2.2-2 is NOT complete.** `MARKETING_SENDING_ENABLED` is absent from production and is a different variable from the Phase C switch (DoD 13). ⚠️ **The migration is NOT applied to production** — the three tables do not exist there yet |
