...
- **Tables (auth path)**: `profiles` (`id`, `onboarded`, `language`, name/avatar/bio; `email` removed from anon-readable columns by a hardening migration), `subscriptions` (Pro check: `status`, `current_period_end`), `user_memory` (seeded at onboarding; `user_id` is **text**, becoming `uuid` with an `auth.users` FK once `20260911b_user_memory_auth_fk.sql` is applied — no client impact), `conversations` (free-limit message counting).
