...
describe('architecture contracts (source)', () => {
  ...
  it('runs INSIDE the tools\' execute(), gated on the frame the route already derived — never as a prefetch', () => {
    ...
    for (const tool of ['searchPlaces(query, location, type, lang, userLocation), travelEditorialFor(location)', 'getHotelPrices(location, checkIn, checkOut, budget?.max, lang), travelEditorialFor(location)', 'travelEditorialFor(destination)', 'webSearch(query, lang), travelEditorialFor(null)']) {
