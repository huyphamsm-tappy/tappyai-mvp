...
describe('Internal UAT — six capability-level flows', () => {
  it('1 · Food & Drink / food_delivery — "Tôi muốn đặt món ăn giao tận nhà."', async () => {
    // A3.3 / A3.6 (2026-09-20): the platforms' SEARCH links are no longer order actions; the venue's
    // OWN page on ShopeeFood (entity-scoped evidence) is the order CTA, direct and attributed.
    const row: Row = { name: 'Phở Hòa', address: '260C Pasteur', maps_link: 'https://maps.google.com/?cid=1', order_links: [{ name: 'ShopeeFood', url: 'https://shopeefood.vn/tim-kiem?q=Ph%E1%BB%9F%20H%C3%B2a' }, { name: 'GrabFood', url: 'https://food.grab.com/vn/en/s?searchKeyword=Ph%E1%BB%9F%20H%C3%B2a' }], order_search_results: [{ title: 'Phở Hòa Pasteur - ShopeeFood', link: 'https://shopeefood.vn/ho-chi-minh/pho-hoa-pasteur', snippet: '', evidence_scope: 'entity', evidence_about: 'Phở Hòa' }] }
    ...
    expect(lead).toMatchObject({ kind: 'order', urlKind: 'direct', url: 'https://shopeefood.vn/ho-chi-minh/pho-hoa-pasteur' })
    const grab = depthProfileForCapability(getProvider('grabfood')!, 'food_delivery')!
    record({ domain: 'Food & Drink', capability: 'food_delivery', userIntent: 'Tôi muốn đặt món ăn giao tận nhà.', providerSelected: 'ShopeeFood (the venue page; Tier 2 direct handoff)', dataSource: 'order_search_results entity-scoped evidence (L3); registry facts grabfood/shopeefood', freshness: null, configuration: null, linkKind: 'DIRECT (venue page)', transactionDepth: `restaurant L3 · guest L${grab.guestDepth} (registry)`, authRequirement: `GrabFood ${grab.authRequiredAt} · ShopeeFood app_only`, handoff: lead ? label(lead) : null, expected: 'food_order/food_delivery → the venue page on GrabFood/ShopeeFood, NOT PasGo, never a search, no false L5', actual: `lead action ${lead?.kind} ${lead?.platform} (${lead?.urlKind}); CCP links on row: 0`, verdict: 'PASS', assertions: a })
  })
