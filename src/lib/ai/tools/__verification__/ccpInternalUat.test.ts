...
describe('Internal UAT — six capability-level flows', () => {
  ...
  it('2 · Food & Drink / table_reservation — "Tôi muốn đặt bàn cho 2 người lúc 19h."', async () => {
    ...
    const result: Row = { results: [row], _tappy_place_domain: 'food', source: 'Google Maps' }
    await attachCommerceLinks('search_places', result, { enabled: true, now: NOW, location: 'Quận 3', userText: 'Tôi muốn đặt bàn cho 2 người lúc 19h.', search: async () => [{ title: 'Nhà hàng Quá Ngon', link: 'https://pasgo.vn/nha-hang/nha-hang-qua-ngon-1234', snippet: '' }] })
    ...
    expect(cfgOf(l)).toEqual({ sfAdult: '2', sfChild: '0', sfDateFrom: '13/09/2026', sfTimeFrom: '19:00' })
    record({ domain: 'Food & Drink', capability: 'table_reservation', userIntent: 'Tôi muốn đặt bàn cho 2 người lúc 19h.', providerSelected: 'pasgo', dataSource: l!.freshness.source + ' + discovery hit (fixture)', freshness: l!.freshness, configuration: cfgOf(l), linkKind: l!.kind, transactionDepth: depthText(l), authRequirement: l!.authRequiredAt, handoff: label(lead), expected: 'table_reservation → PasGo, restaurant/date/time/party config, L5 guest, no login claimed', actual: `${l!.destinationUrl} · assumed ${l!.assumedParams.join(',') || 'none'} · expires ${l!.expiresAt}`, verdict: 'PASS', assertions: a })
  })
