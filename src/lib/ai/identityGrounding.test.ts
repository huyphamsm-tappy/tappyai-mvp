...
describe('the detector reports and never rewrites', () => {
  ...
  it('the route only logs the finding', () => {
    const use = route.slice(route.indexOf('const ungrounded = evidence.ungroundedNames'))
      .slice(0, 700)
    expect(use).toContain('tappyai_identity_ungrounded')
    expect(use).not.toContain('return new Response')
  })
})
