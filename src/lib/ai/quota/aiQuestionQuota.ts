...
//               same "today" copy `/api/chat` gives a visitor on this branch. (The phase4 branch
//               made this a LIFETIME trial on 2026-09-15 together with a chat-route refactor;
//               that pair of changes is not on canonical, and this module follows the branch it
//               ships on. The wire shape still carries `period` so a client renders either.)
...
//
// 🚨 SCOPE ON CANONICAL. `/api/chat` here still meters through its own counters
// (`anon_chat_usage_increment` + the `tappy_anon` cookie, and a count of today's `conversations`
// rows). Those cannot be shared with a Scam Shield analysis (which is not a conversation row), so
// on this branch this module is the pool for the AI features that spend THROUGH it — today,
// Analyze Message. Unifying chat onto it is the phase4 chat refactor, a separate decision.
//
// ============================================================================
...
// WHO IS WHO
// ============================================================================
//   user   a registered account (`user.is_anonymous !== true`)         → 15/day, keyed by id
...
function bucket(identity: AiQuotaIdentity): { key: string; windowMs: number; limit: number; period: AiQuotaPeriod } {
  ...
  return { key: `ai:q:${prefix}:${identity.id}:${vnToday()}`, windowMs: ONE_DAY_MS, limit: ANON_DAILY_LIMIT, period: 'day' }
}
