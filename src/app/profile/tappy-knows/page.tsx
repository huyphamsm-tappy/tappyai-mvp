...
import { RefreshCw, Trash2, Brain, MapPin, Users, Clock, Sparkles, UtensilsCrossed, Heart } from 'lucide-react'
...
function TagList({ items, color = 'primary' }: { items: string[]; color?: string }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-2">
      {items.map((t, i) => (
        <span key={i} className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${colorMap[color] ?? colorMap.primary}`}>
          {t}
        </span>
      ))}
    </div>
          ...
          <div className="space-y-4">
            ...
            {prefs.food && prefs.food.length > 0 && (
              <MemoryCard icon={UtensilsCrossed} label="Ẩm thực yêu thích" iconColor="text-orange-500">
                <TagList items={prefs.food} color="orange" />
              </MemoryCard>
            )}
            ...
            {prefs.shopping && prefs.shopping.length > 0 && (
              <MemoryCard icon={Sparkles} label="Mua sắm" iconColor="text-green-500">
                <TagList items={prefs.shopping} color="green" />
              </MemoryCard>
            )}
            ...
            {prefs.avoid && prefs.avoid.length > 0 && (
              <MemoryCard icon={UtensilsCrossed} label="Không thích / Kiêng" iconColor="text-red-400">
                <TagList items={prefs.avoid} color="red" />
              </MemoryCard>
            )}
              ...
              <div className="card p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {memory.history.slice(-8).reverse().map((h, i) => (
                    <span
                      key={i}
                      className="inline-block px-2.5 py-1 rounded-full text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300"
                    >
                      {h}
                    </span>
                  ))}
