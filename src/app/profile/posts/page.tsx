  ...
  return (
    <div className="min-h-dvh bg-gray-50 dark:bg-gray-950 pb-24">
      ...
      <div className="sticky top-0 z-20 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="font-bold text-gray-900 dark:text-white flex-1">{t('posts.title')}</h1>
        <span className="text-sm text-gray-400">{t('posts.count', { n: String(reviews.length) })}</span>
      </div>
      ...
      {reviews.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-gray-400">
          ...
          <Link href="/reviews/new" className="mt-4 bg-interactive text-white px-5 py-2 rounded-full text-sm font-semibold">{t('posts.firstCta')}</Link>
        </div>
      ) : (
            ...
            <div className="fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-gray-900 rounded-t-3xl px-5 pt-4 pb-8 max-w-2xl mx-auto shadow-2xl">
              ...
              <div className="flex items-center gap-3 mb-5 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 line-clamp-2 mt-0.5">{r.body || t('posts.photoPost')}</p>
                  <p className="text-xs text-gray-400 mt-1">{r.is_hidden ? t('posts.hidden') : t('posts.visible')} · ❤️ {r.like_count} · 💬 {r.comment_count}</p>
                </div>
              </div>
              <div className="space-y-2">
                <button onClick={() => handleToggleHide(r.id, r.is_hidden)}
                  ...
                  {r.is_hidden ? t('posts.show') : t('posts.hide')}
                </button>
