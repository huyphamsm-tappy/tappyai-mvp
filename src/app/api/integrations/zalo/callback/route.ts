...
export async function GET(req: NextRequest) {
  ...
  // set by the initiator. Without this, an arbitrary `state` could attach a Zalo
  // account to a victim's TappyAI account (the write uses the admin client).
  const { user } = await getRequestUser(req)
  if (!user || user.id !== state) {
    return NextResponse.redirect(new URL('/profile/integrations?error=zalo_state', req.url))
  }
