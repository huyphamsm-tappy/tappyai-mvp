...
beforeEach(() => {
  ...
  h.consume.mockImplementation(async (identity: { kind: string }) => {
    // Canonical: every identity is metered per VN day (anonymous 5, registered 15).
    const limit = identity.kind !== 'user' ? 5 : 15
    return { ok: h.state.quotaOk, limit, period: 'day', used: h.state.quotaOk ? 1 : limit, remaining: h.state.quotaOk ? limit - 1 : 0, scope: 'instance' }
  })
...
describe('the allowance — ONE shared Tappy AI question pool, no Scam Alerts quota of its own', () => {
  ...
  it('an anonymous session spends from the anonymous daily allowance, keyed by the verified anonymous id', async () => {
    h.state.user = { id: 'anon-123', is_anonymous: true }
    const res = await post({ text: TELEGRAM_SCAM })
    expect(h.consume).toHaveBeenCalledWith({ kind: 'anon', id: 'anon-123' })
    expect((await res.json()).quota).toMatchObject({ kind: 'anon', limit: 5, period: 'day' })
  })
