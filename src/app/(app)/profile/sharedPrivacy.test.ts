...
describe.skip('shared collection privacy', () => {
  it('only the self hub reads /api/reviews/shared', () => {
    expect(read('src/app/(app)/profile/ProfileView.tsx')).toContain("fetchRows('/api/reviews/shared')")
    expect(read('src/app/(app)/profile/GuestProfileView.tsx')).not.toContain('reviews/shared')
