...
/** Tu-vi By-Year + Lifetime data, transcribed VERBATIM from prod fortuneEngine.ts (worktree
 *  cool-vaughan-b3c7ff). Order/counts must match web exactly for byte-identical readings. */

internal val YEAR_ANIMALS: List<String> = listOf(
    "Tý", "Sửu", "Dần", "Mão", "Thìn", "Tỵ", "Ngọ", "Mùi", "Thân", "Dậu", "Tuất", "Hợi",
)

/** Web is a Record<number,...> accessed as YEAR_COMPAT[diff] where diff = 0..11.
 *  Emitted here as a List indexed by that numeric key (0..11), so YEAR_COMPAT[diff]
 *  maps 1:1 — NOT the literal declaration order of the web object. */
internal val YEAR_COMPAT: List<YearCompat> = listOf(
    YearCompat(label = "⚠ Năm bản mệnh", note = "Năm cầm tinh trùng với tuổi bạn — vận khí biến động, cần thận trọng trong các quyết định lớn, nhưng cũng là năm để thử thách và khẳng định lại bản thân."),
    YearCompat(label = "—", note = "Năm vận khí đi ngang, phù hợp để tích lũy kinh nghiệm và chuẩn bị kế hoạch vững chắc hơn cho chu kỳ tiếp theo."),
    YearCompat(label = "🟡 Lục hợp", note = "Lục hợp — năm có lợi thế trong hợp tác; các kế hoạch chung hoặc hợp tác kinh doanh đôi bên dễ thành công hơn làm đơn lẻ."),
    YearCompat(label = "🔸 Tương hình", note = "Tương hình — cần chú ý các mối quan hệ dễ xảy ra bất đồng và xích mích; kiên nhẫn, dĩ hòa vi quý là cách ứng xử tốt nhất."),
    YearCompat(label = "✨ Tam hợp", note = "Tam hợp cục — năm rất thuận lợi, quý nhân phù trợ nhiều, sự nghiệp và tình duyên đều có cơ hội bứt phá mạnh."),
    YearCompat(label = "—", note = "Năm trung bình, nên củng cố những gì đang có thay vì mở rộng thêm — tập trung vào chất lượng hơn số lượng."),
    YearCompat(label = "⚡ Lục xung", note = "Lục xung — năm nhiều xáo trộn và biến động, giữ vững lập trường và tránh các quyết định quá liều lĩnh, đặc biệt trong nửa năm đầu."),
    YearCompat(label = "—", note = "Năm khá thuận về tài lộc và sức khỏe, thích hợp để chú trọng các mối quan hệ thân thiết và đầu tư cho gia đình."),
    YearCompat(label = "✨ Tam hợp", note = "Tam hợp cục — vận khí hanh thông toàn diện, thích hợp để mở rộng quy mô, đầu tư và gặt hái thành quả tích lũy."),
    YearCompat(label = "🔸 Tương hình", note = "Tương hình — cẩn thận trong giao tiếp và ký kết hợp đồng, tránh tranh chấp không cần thiết."),
    YearCompat(label = "—", note = "Năm thuận lợi cho học tập và phát triển bản thân, những nỗ lực này sẽ cho quả tốt trong tương lai."),
    YearCompat(label = "—", note = "Năm tiệm cận chu kỳ mới — nên kết thúc những việc còn dang dở và chuẩn bị tâm thế đón giai đoạn biến chuyển sắp tới."),
)

internal val YEAR_BANKS: FortuneBanks = FortuneBanks(
    love = listOf(
    ...
    luckyNumbers = listOf(1, 3, 6, 8, 9, 18, 36),
    luckyColors = listOf("Vàng", "Đỏ", "Xanh lá", "Trắng", "Tím", "Xanh dương", "Cam"),
)

internal val MONTH_BANKS: MonthBanks = MonthBanks(
    love = listOf(
...
internal val STAGE_DEFS: List<StageDef> = listOf(
...
internal val STAGE_BANKS: Map<String, StageBank> = mapOf(
    "nienhieu" to StageBank(
        fate = listOf(
