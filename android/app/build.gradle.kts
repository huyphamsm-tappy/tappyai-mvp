...
// Execution-time gate for the values that have no safe default: credentials and signing.
//
// The public endpoints (API_BASE_URL_RELEASE, WEB_APP_URL) are deliberately NOT listed here any
// more. They now default to production and are validated by the placeholder guard at the top of
// this file, which checks the resolved *value* rather than the property's presence — so a build
// that simply omits them is correct, while one that sets them to a placeholder still fails, and
// fails at configuration time rather than after the compile.
//
// What remains has no defensible default: shipping without real credentials produces an app with
// no working login, and shipping without the keystore produces an UNSIGNED artifact that Play
// rejects. Kept at `doFirst` so release-variant lint, unit tests and generateReleaseBuildConfig
// still run on a machine that has neither.
tasks.matching { it.name in setOf("assembleRelease", "bundleRelease") }.configureEach {
    doFirst {
        ...
        if (missing.isNotEmpty()) {
            throw GradleException(
                "Refusing to build a release APK/AAB with missing credentials or signing config.\n" +
                    missing.joinToString("\n") { "  • $it" } +
                    "\n\nSupply them via ~/.gradle/gradle.properties or -P<name>=...\n" +
                    "See docs/release/ANDROID_RELEASE_CONFIGURATION.md"
            )
        }
    }
