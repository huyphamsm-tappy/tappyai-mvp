...
let YEAR_ANIMALS = ["Tý", "Sửu", "Dần", "Mão", "Thìn", "Tỵ", "Ngọ", "Mùi", "Thân", "Dậu", "Tuất", "Hợi"]
...
let YEAR_COMPAT: [Int: YearCompat] = [
    0:  YearCompat(label: "⚠ Năm bản mệnh", note: "Năm cầm tinh trùng với tuổi bạn — vận khí biến động, cần thận trọng trong các quyết định lớn, nhưng cũng là năm để thử thách và khẳng định lại bản thân."),
    4:  YearCompat(label: "✨ Tam hợp", note: "Tam hợp cục — năm rất thuận lợi, quý nhân phù trợ nhiều, sự nghiệp và tình duyên đều có cơ hội bứt phá mạnh."),
    8:  YearCompat(label: "✨ Tam hợp", note: "Tam hợp cục — vận khí hanh thông toàn diện, thích hợp để mở rộng quy mô, đầu tư và gặt hái thành quả tích lũy."),
    6:  YearCompat(label: "⚡ Lục xung", note: "Lục xung — năm nhiều xáo trộn và biến động, giữ vững lập trường và tránh các quyết định quá liều lĩnh, đặc biệt trong nửa năm đầu."),
    3:  YearCompat(label: "🔸 Tương hình", note: "Tương hình — cần chú ý các mối quan hệ dễ xảy ra bất đồng và xích mích; kiên nhẫn, dĩ hòa vi quý là cách ứng xử tốt nhất."),
    9:  YearCompat(label: "🔸 Tương hình", note: "Tương hình — cẩn thận trong giao tiếp và ký kết hợp đồng, tránh tranh chấp không cần thiết."),
    2:  YearCompat(label: "🟡 Lục hợp", note: "Lục hợp — năm có lợi thế trong hợp tác; các kế hoạch chung hoặc hợp tác kinh doanh đôi bên dễ thành công hơn làm đơn lẻ."),
    1:  YearCompat(label: "—", note: "Năm vận khí đi ngang, phù hợp để tích lũy kinh nghiệm và chuẩn bị kế hoạch vững chắc hơn cho chu kỳ tiếp theo."),
    5:  YearCompat(label: "—", note: "Năm trung bình, nên củng cố những gì đang có thay vì mở rộng thêm — tập trung vào chất lượng hơn số lượng."),
    7:  YearCompat(label: "—", note: "Năm khá thuận về tài lộc và sức khỏe, thích hợp để chú trọng các mối quan hệ thân thiết và đầu tư cho gia đình."),
    10: YearCompat(label: "—", note: "Năm thuận lợi cho học tập và phát triển bản thân, những nỗ lực này sẽ cho quả tốt trong tương lai."),
    11: YearCompat(label: "—", note: "Năm tiệm cận chu kỳ mới — nên kết thúc những việc còn dang dở và chuẩn bị tâm thế đón giai đoạn biến chuyển sắp tới."),
]

let YEAR_BANKS = FortuneBanks(
    love: [
...
let STAGE_DEFS: [StageDef] = [
    StageDef(key: "nienhieu", label: "Thời niên thiếu", ageRange: "0 – 18 tuổi", emoji: "🌱"),
    StageDef(key: "thanhnien", label: "Thanh niên", ageRange: "18 – 30 tuổi", emoji: "🔥"),
    StageDef(key: "trungnien", label: "Trung niên", ageRange: "30 – 50 tuổi", emoji: "🌳"),
    StageDef(key: "hauvan", label: "Hậu vận", ageRange: "50+ tuổi", emoji: "🌟"),
]
...
let STAGE_BANKS_DATA: [String: StageBanks] = [
    "nienhieu": StageBanks(
        fate: [
